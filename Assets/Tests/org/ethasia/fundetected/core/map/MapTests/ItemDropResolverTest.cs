...
namespace Org.Ethasia.Fundetected.Core.Map.Tests
{
    public class ItemDropResolverTest
    {
        ...
        public void TestResolveItemDropPicksLastRowIfNoOtherHit()
        {
            ...
            Assert.That(result, Is.Not.Null);
            Assert.That(result.Value.Item.Name, Is.EqualTo("First Item Fifth Row"));
        }

        [Test]
        public void TestResolveItemDropDoesNotFindItemIfRollIsHigherThanAllItemChances()
        {
        ...
        [Test]
        public void TestResolveItemDropDoesNotDropItemWhenTooHighMinimumItemLevel()
        {
            int[] randomNumbersToGenerate = { 223 };
            float[] randomFloatsToGenerate = { };
            double[] randomDoublesToGenerate = { 0.04, 0.2 };

            rngMock.Reset(randomNumbersToGenerate, randomFloatsToGenerate, randomDoublesToGenerate);

            List<DropTable> testInput = TestDropTableFactory.CreateDropTableWithFourJewelryItems();
            DropTableEntry? result = ItemDropResolver.ResolveItemDrop(testInput, 1);

            Assert.That(result, Is.Null);
        }

        [Test]
        public void TestResolveItemDropDropsItemWhenItemLevelIsMet()
        {
            int[] randomNumbersToGenerate = { 223 };
            float[] randomFloatsToGenerate = { };
            double[] randomDoublesToGenerate = { 0.04, 0.2 };
            ...
            List<DropTable> testInput = TestDropTableFactory.CreateDropTableWithFourJewelryItems();
            DropTableEntry? result = ItemDropResolver.ResolveItemDrop(testInput, 4);
        ...
        [Test]
        public void TestResolveItemDropPicksItemWithHigherDropChanceIfTheLowerHastTooHighItemLevel()
        {
            int[] randomNumbersToGenerate = { 223, 0 };
            float[] randomFloatsToGenerate = { };
            double[] randomDoublesToGenerate = { 0.01, 0.3 };

            rngMock.Reset(randomNumbersToGenerate, randomFloatsToGenerate, randomDoublesToGenerate);

            List<DropTable> testInput = TestDropTableFactory.CreateDropTableWithFourJewelryItems();
            DropTableEntry? result = ItemDropResolver.ResolveItemDrop(testInput, 2);

            Assert.That(result, Is.Not.Null);
            Assert.That(result.Value.Item.Name, Is.EqualTo("Second Item Second Row"));
        }
    }
}   